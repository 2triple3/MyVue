...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			multiselect: false,
			multikey: false,
			multirow:false,			//add by wl
			multirowSel:false,		//add by wl
			multirowSelFlag:false,//add by wl
			multiEnable:true,		//add by wl
			checkBoxShowFlag:true,//add by wulei 2012-03-15
			editurl: null,
			search: false,
			caption: "",
			hidegrid: true,
		...
		addMulti = function(rowid,pos,irow){
			if( ts.p.multiEnable ){
				...
				var	v = "<input type=\"checkbox\""+" id=\"jqg_"+rowid+"\" class=\"cbox\" name=\"jqg_"+rowid+"\" disabled=\"disabled\" />",
				prp = formatCol(pos,irow);
			}
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if(ts.p.pginput===true) {
					...
					$('#sp_1',pgboxes).html($.fmatter ? $.fmatter.util.NumberFormat(ts.p.lastpage,{}):ts.p.lastpage);//modify by wulei ata 2012-2-10
		...
		if( this.p.multiEnable ){
			if(this.p.multiselect) {
				if (p.checkBoxShowFlag) {
					this.p.colNames.unshift("<input id='cb_"+this.p.id+"' class='cbox' type='checkbox'/>");
				} else {
					this.p.colNames.unshift("<input id='cb_"+this.p.id+"' class='cbox' type='hidden'/>");
				}
			...
			if(this.p.multiselect) {
				if (p.checkBoxShowFlag) {
					this.p.colNames.unshift("<input id='cb_"+this.p.id+"' class='cbox' type='checkbox' disabled='disabled'/>");
				} else {
					this.p.colNames.unshift("<input id='cb_"+this.p.id+"' class='cbox' type='hidden' disabled='disabled'/>");
				}
				this.p.colModel.unshift({name:'cb',width:isSafari ? ts.p.multiselectWidth+ts.p.cellLayout : ts.p.multiselectWidth,sortable:false,resizable:false,hidedlg:true,search:false,align:'center',fixed:true});
			}
		}
						...
						if(scb){$(ts).jqGrid("setSelection",ptr[0].id,true);}
							...
							if( !ts.p.multirow){
								if( !ts.p.multirowSel){
									...
									$(ts).jqGrid("setGridParam",{multirowSelFlag:true});
									$(ts).jqGrid("setSelection",ptr[0].id,true);
									$(ts).jqGrid("setGridParam",{multirowSelFlag:false});
								}
							}
						}
					} else {
	...
	setSelection : function(selection,onsr) {
		return this.each(function(){
			...
			if(!$t.p.multiselect) {
				...
				if($t.p.multirowSel && $t.p.multirowSelFlag){//add by wl
					if(pt.className !== "ui-subgrid") {
					if( $t.p.selrow ) {$("tr#"+$.jgrid.jqID($t.p.selrow),$t.grid.bDiv).removeClass("ui-state-highlight").attr("aria-selected","false") ;}
					$t.p.selrow = pt.id;
					$(pt).addClass("ui-state-highlight").attr("aria-selected","true");
					if( $t.p.onSelectRow && onsr) { $t.p.onSelectRow.call($t,$t.p.selrow, true,false); }
					}
				}else if( !$t.p.multirowSelFlag ){//add by wl
					$t.p.selrow = pt.id;
					...
					if (  ia === -1 ){ 
						...
						if( $t.p.onSelectRow && onsr) { $t.p.onSelectRow.call($t,$t.p.selrow, stat,true); }//add by wl
					} else {
						...
						//if( $t.p.onSelectRow && onsr) { $t.p.onSelectRow.call($t,$t.p.selrow, stat); }
						if( $t.p.onSelectRow && onsr) { $t.p.onSelectRow.call($t,$t.p.selrow, stat,false); }//add by wl
				...
				while(k < datalen) {
					...
					if(gi) {
						if( this.p.multiEnable ){
							...
							v = "<input type=\"checkbox\""+" id=\"jqg_"+rowid+"\" class=\"cbox\" disabled=\"disabled\"/>";
						}
						prp = t.formatCol(ni,1);
