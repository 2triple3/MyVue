...
My97DP.prototype = {
    ...
    splitDate: function (M, F, O, _, D, B, A, N, G) {
        ...
        if (M && M.loadDate) {
            ...
            if (M != "") {
                F = F || $dp.dateFmt;
                var K, P = 0,
                    J, C = /yyyy|yyy|yy|y|MMMM|MMM|MM|M|dd|d|%ld|HH|H|mm|m|ss|s|DD|D|WW|W|w/g,
                    Q = F.match(C);
                C.lastIndex = 0;
                if (G) {
                    J = M.split(/\W+/);
    ...
    update: function () {
        ...
        if ($dp.el[$dp.elProp] != $) {
            $dp.el[$dp.elProp] = $;
         	$dp.win.$($dp.el).change(); //[Bugfree_1726_日期输入非格式化形式如（2013-9-13）Onchange()方法不调用] fanr 2013-10-14
        }
        this.setRealValue();
    },
        ...
        for (_ = 1, F = C; _ < 7; _++) {
            ...
            for ($ = 0; $ < 7; $++) {
                ...
                if ($dp.isShowWeek && $ == 0 && (_ < 4 || J)) {
                    G.a("<td class=Wweek>" + getWeek(K, $dp.firstDayOfWeek == 0 ? 1 : 0) + "</td>");
                }
                G.a("<td ");
                if (J) {
    ...
    _fillQS: function ($) {
        ...
        var C = this.QS,
            B = C.style,
            A = new sb();
        A.a("<table class=WdayTable width=100% height=100% border=0 cellspacing=0 cellpadding=0>");
        ...
        for (var _ = 0; _ < C.length; _++) {
            if (C[_]) {
                A.a("<tr><td style='text-align:left' nowrap='nowrap' class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onclick=\"");
...
function _tab(G) {
    ...
    var TEMP = "";
    if ($dp.enableInputMask && !$OPERA && !$dp.readOnly && $c.currFocus == $dp.el && (Q >= 48 && Q <= 57)&&$dp.ebableAutoComplete) {
        var N = $dp.el,
            S = N.value,
            A = _(N),
            K = {
            ...
            W = 0,
            I = 0,
            L, V = /yyyy|yyy|yy|y|MM|M|dd|d|%ld|HH|H|mm|m|ss|s|WW|W|w/g,
            F = $dp.dateFmt.match(V),
            D, C, X, P, O, B,L = 0;
            //alert('S-1-|'+S);
        if (S != "") {
            I = S.match(/[0-9]/g);
        ...
        //alert('S-4-|'+S+'|====|'+(S=='-'));
        if( S.indexOf('-') == 0 && S.substring(0,1) == "-" )  TEMP = "-";
        if( S.indexOf('+') == 0 && S.substring(0,1) == "+" )  TEMP = "+";
        //alert('S-TEMP-|'+TEMP);
        S = "";
        V.lastIndex = 0;
        while ((M = V.exec($dp.dateFmt)) !== null) {
            ...
            if (K.arr[W] || C == "M" && O > 12 || C == "d" && O > 31 || C == "H" && O > 23 || "ms".indexOf(C) >= 0 && O > 59) {
                if (M[0].length == 2) {
                    D = "0" + X;
                    B = 1;
                } else {
